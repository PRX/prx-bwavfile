    ...
    W: Write + Seek,
{
    ...
    ///
    /// Bypasses the [`audio_frame_writer`](Self::audio_frame_writer)
    /// PCM-frame path (which inserts an `elm1` 0x4000-aligned filler
    /// before the data chunk — correct for PCM, wrong for MPEG/codec
    /// data). The bytes are written verbatim; RIFF odd-length pad-byte
    /// handling is applied automatically.
    ///
