...
/// `A=MPEG{ver}L{layer},F={sample_rate},B={bit_rate_kbps},M={mode},T=bwavfile\r\n`.
...
///
/// Format: `A=MPEG{version}L{layer},F={sample_rate},B={bit_rate_kbps},M={mode},T=bwavfile\r\n`
fn default_coding_history(info: &MpegInfo) -> String {
    format!(
        "A=MPEG{}L{},F={},B={},M={},T=bwavfile\r\n",
...
mod tests {
    ...
    fn synthesizes_coding_history_when_bext_is_empty() {
        ...
        // Synthesized: starts with "A=MPEG1L2," and ends with the tool tag.
        assert!(
        ...
        assert!(
            parsed.coding_history.contains("T=bwavfile"),
            "expected bwavfile tool tag, got: {}",
            parsed.coding_history
        );
    }
    ...
    fn preserves_user_supplied_coding_history() {
        ...
        let mut bext = sample_bext();
        bext.coding_history = custom_history.clone();
