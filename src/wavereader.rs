...
impl<R: Read + Seek> AudioFrameReader<R> {
    ...
    pub fn new(mut inner: R, format: WaveFmt, start: u64, length: u64) -> Result<Self, Error> {
        ...
        // channel_count`. Callers wanting raw bytes should use
        // `WaveReader::read_data_chunk` instead.
...
impl<R: Read + Seek> WaveReader<R> {
    ...
    ///
    /// For PCM files these are raw sample bytes (use
    /// [`audio_frame_reader`](Self::audio_frame_reader) instead for
    /// sample-level access). For codec-encoded data (MPEG, ADPCM,
    /// A-Law, etc.) these are the codec body, which a caller can hand
    /// to a separate codec implementation for decoding.
    ///
