...
mod tests {
    ...
    fn round_trip_fully_populated() {
        let cart = Cart {
            ...
            producer_app_id: "prx-bwavfile".to_string(),
            producer_app_version: "0.1.0".to_string(),
            ...
            reserved: [0u8; 276],
            url: "https://www.prx.org/".to_string(),
    ...
    fn reserved_bytes_round_trip_verbatim() {
        ...
        // round-trip (the JS implementation reads this as a string,
        // which corrupts non-ASCII bytes; we use [u8; 276] to preserve).
    ...
    fn tag_text_odd_length() {
        // Odd length triggers the RIFF pad-byte path.
        round_trip(Cart {
            tag_text: "x".repeat(7),
            ..Cart::default()
        });
    ...
    fn round_trip_via_wave_writer() {
        let cart = Cart {
            title: "WaveWriter Roundtrip".to_string(),
            ..Cart::default()
        };
        ...
        let parsed = reader
            .cart()
            .unwrap()
            .expect("cart chunk should be present");
        assert_eq!(parsed, cart);
    }

    #[test]
    fn no_cart_chunk_returns_none() {
        let mut reader = WaveReader::open("tests/media/pt_24bit.wav").unwrap();
        assert!(reader.cart().unwrap().is_none());
    }

    #[test]
    fn read_cc_0101_wav_has_cart() {
        // cc_0101.wav is the public AES46 cart chunk reference from cartchunk.org.
        let mut reader = WaveReader::open("tests/media/cc_0101.wav").unwrap();
        let cart = reader
            .cart()
            .unwrap()
            .expect("cc_0101.wav must have a cart chunk");
        assert_eq!(cart.version, "0101");
        // The reference file should have a non-empty title.
        assert!(
            !cart.title.is_empty(),
            "cc_0101.wav title should not be empty"
        );
    }

    #[test]
    fn read_60315_wav_has_cart() {
        // 60315.wav is a real-world broadcast WAV with cart chunk.
        let mut reader = WaveReader::open("tests/media/60315.wav").unwrap();
        let cart = reader
            .cart()
            .unwrap()
            .expect("60315.wav must have a cart chunk");
        assert_eq!(cart.version, "0101");
    }

    #[test]
    fn rewrite_60315_cart_preserves_structure() {
        // Read cart from real broadcast file, write to a fresh WAVE,
        // re-read, assert structural equality. This is the key test
        // for byte-exact `reserved` round-trip — string-based
        // round-tripping would corrupt non-ASCII bytes a real cart
        // chunk may contain.
        let mut reader = WaveReader::open("tests/media/60315.wav").unwrap();
        let original_cart = reader.cart().unwrap().unwrap();

        let mut cursor = Cursor::new(Vec::<u8>::new());
        let format = WaveFmt::new_pcm_mono(48000, 24);
        let mut w = WaveWriter::new(&mut cursor, format).unwrap();
        w.write_cart(&original_cart).unwrap();
        let mut frame_writer = w.audio_frame_writer().unwrap();
        frame_writer.write_frames(&[0i32]).unwrap();
        frame_writer.end().unwrap();

        cursor.seek(SeekFrom::Start(0)).unwrap();
        let mut re_reader = WaveReader::new(cursor).unwrap();
        let re_cart = re_reader.cart().unwrap().unwrap();
        assert_eq!(re_cart, original_cart);
    }
}
