...
mod tests {
    ...
    /// Helper: hand-construct an MpegInfo for a typical MP2 broadcast
    /// stream (CBR 256 kbps, 44.1 kHz, stereo, Layer II). Avoids
    /// needing a real MP2 fixture in the test corpus.
    fn sample_info() -> MpegInfo {
        MpegInfo {
            version: MpegVersion::V1,
            layer: MpegLayer::Layer2,
            bit_rate: 256,
            sample_rate: 44100,
    ...
    #[test]
    fn mpeg_version_r98_label() {
        assert_eq!(MpegVersion::V1.r98_label(), "1");
        assert_eq!(MpegVersion::V2.r98_label(), "2");
        assert_eq!(MpegVersion::V25.r98_label(), "2.5");
    }

    #[test]
    fn mpeg_layer_helpers() {
        assert_eq!(MpegLayer::Layer1.number(), 1);
        assert_eq!(MpegLayer::Layer2.number(), 2);
        assert_eq!(MpegLayer::Layer3.number(), 3);
        assert_eq!(MpegLayer::Layer1.head_layer(), 1);
        assert_eq!(MpegLayer::Layer2.head_layer(), 2);
        assert_eq!(MpegLayer::Layer3.head_layer(), 4);
    }

    #[test]
    fn channel_mode_helpers() {
        ...
        assert_eq!(ChannelMode::Stereo.head_mode(), 1);
        assert_eq!(ChannelMode::JointStereo.head_mode(), 2);
        assert_eq!(ChannelMode::DualMono.head_mode(), 4);
        assert_eq!(ChannelMode::Mono.head_mode(), 8);
        assert_eq!(ChannelMode::Stereo.r98_label(), "stereo");
        assert_eq!(ChannelMode::JointStereo.r98_label(), "joint-stereo");
        assert_eq!(ChannelMode::DualMono.r98_label(), "dual-mono");
        assert_eq!(ChannelMode::Mono.r98_label(), "mono");
    }

    #[test]
    fn sample_info_constructs() {
    ...
    fn parse_test_id3_mp2() {
        ...
        // Audio body matches test.mp2 except for the prefix offset.
    ...
    fn parse_test_bad_mp2() {
        ...
        // test-bad.mp2 is intentionally malformed — should either
        // fail to parse or be flagged as non-homogeneous.
        assert!(
            result.is_err() || matches!(&result, Ok(info) if !info.homogeneous),
            "test-bad.mp2 should fail or be non-homogeneous; got {:?}",
            result.as_ref().ok()
        );
    }
